...
import Coupon from "./backend/order-service/models/couponModel.js";
import {
  createOrderService,
  getOrdersService,
  getOrderByIdService,
...
} from "./backend/order-service/services/orderService.js";

const Payment = require("./backend/payment-service/models/PaymentModel.js");
...
async function runMatrix() {
  ...
  try {
    ...
    const ord2 = await createOrderService(
      {
        restaurantId,
        items: [{ foodId: foodItem._id.toString(), quantity: 2, price: 1 }],
        deliveryAddress: "123 Lê Lợi, Q1, TP.HCM",
      },
      customerA
    );
    ...
    // 5. Customer A creates order with quantity = -1 or 1.5 -> 400 Bad Request
    ...
    try {
      await createOrderService(
        {
          restaurantId,
          items: [{ foodId: foodItem._id.toString(), quantity: 1.5, price: 65000 }],
          deliveryAddress: "Hà Nội",
        },
        customerA
      );
    } catch (e) {
      if (e.statusCode === 400) rejectedFraction = true;
    }
    assertTest(5, "Validation: Order with quantity < 0 or fractional (1.5) rejected with 400 Bad Request", rejectedNeg && rejectedFraction);
